        ...
        try {  
            ...
            for(FileItem fileItem : list){  
                if (fileItem.isFormField()) {//是文本字段  
                    ...
                    String image = fileItem.getName(); //获取文件路径加名称
                    int index = image.lastIndexOf("/");//获取文件最后一个/的下标
                    System.out.println(index);
                    String imageName=image.substring(index+1,image.length());//获取上传图片的名称
                    //String imageName1=imageName.substring(imageName.length()-5,imageName.length());
                   ...
                   String upload_dir = request.getServletContext().getRealPath("/upload");//获取服务器端 /upload 路径  
                    System.out.println(upload_dir+"123");
